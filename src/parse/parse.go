...
func (t *Tree) parse(tree *Tree) {
	...
	for t.peek(1).Type != itemEOF {
		...
		case itemSpace:
			...
			if n == nil {
				// The calculated indent level was the same as the current indent level. Future items will be
		...
		case itemDefinitionTerm:
			...
			t.indentLevel++
		case itemBullet:
			// FIXME: This will get fixed when I am ready for full bullet list support.
			var nn Node
...
func (t *Tree) definitionList(i *item) Node {
	...
	// backup so the parser will get the same token and on the next go around the token will be added to the NodeList if
	// the definition list created here.
	t.backup()
	// Tell the parser we have an open definition list. Future definition terms will be appended to the definition list.
	return sec
}
